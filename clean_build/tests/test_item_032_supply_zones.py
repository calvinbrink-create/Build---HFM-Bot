...
from cipherfx_clean.requirement_runtime import verify_c017_supply_zones
...
def _supply(*future):
    ...
    return next(zone for zone in identify_zones(rows) if zone.kind == "SUPPLY" and zone.origin_index == 0)


def test_supply_zone_lifecycle_reaches_all_four_states_and_later_failure_wins():
    fresh = _supply()
    tested = _supply(_candle(2, 8, 9.5, 7.5, 8.5))
    mitigated = _supply(_candle(2, 9, 11.2, 8.5, 10))
    failed = _supply(
        _candle(2, 9, 11.2, 8.5, 10),
        _candle(3, 10, 11.6, 9.8, 11.2),
    )
...
def test_c017_supply_zone_contract_reports_exact_lifecycle():
    zones = (
        _supply(),
        _supply(_candle(2, 8, 9.5, 7.5, 8.5)),
        _supply(_candle(2, 9, 11.2, 8.5, 10)),
        _supply(_candle(2, 10, 11.6, 9.8, 11.2)),
    )
    ...
    result = verify_c017_supply_zones(report)
    assert result["supply_zone_count"] == 4
