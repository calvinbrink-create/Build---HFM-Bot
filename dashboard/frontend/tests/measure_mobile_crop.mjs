...
await context.addInitScript(({ sessionToken }) => {
  ...
  localStorage.setItem("cipherfx_dashboard_auth_v1", JSON.stringify({ email: "mkt6@vps", expiresAt: Date.now() + 3600000, dashboards: ["mt5"] }));
}, { sessionToken: token });
const page = await context.newPage();
await page.goto(`${DASHBOARD_URL}?mkt6=${Date.now()}`, { waitUntil: "networkidle", timeout: 30000 });
...
await page.screenshot({ path: "/tmp/claude-0/-root/9dca97ce-58d8-4690-a34d-0295150b0558/scratchpad/marketing/overview-mobile-fresh.png" });
await browser.close();
